...
TEST(DiceOpsTest, KnownAnswerZeroInput) {
  ...
  DumpState(CertificateType_Cbor, KeyType_MLDSA_65, "zero_input", next_state);
  ...
  ASSERT_EQ(sizeof(dice::test::kExpectedCborMldsa65Cert_ZeroInput),
            next_state.certificate_size);
  // Comparing everything except for the signature, since ECDSA signatures are
  // not deterministic
  EXPECT_EQ(0,
            memcmp(dice::test::kExpectedCborMldsa65Cert_ZeroInput,
...
TEST(DiceOpsTest, KnownAnswerHashOnlyInput) {
  ...
  DiceResult result = DiceMainFlow(
      ...
      &next_state.certificate_size, next_state.cdi_attest, next_state.cdi_seal);
  EXPECT_EQ(kDiceResultOk, result);
  DumpState(CertificateType_Cbor, KeyType_MLDSA_65, "hash_only_input",
            next_state);
  ASSERT_EQ(sizeof(next_state.cdi_attest),
  ...
  EXPECT_EQ(
      0, memcmp(next_state.cdi_seal, dice::test::kExpectedCdiSeal_HashOnlyInput,
                DICE_CDI_SIZE));
  ASSERT_EQ(sizeof(dice::test::kExpectedCborMldsa65Cert_HashOnlyInput),
            next_state.certificate_size);
  EXPECT_EQ(0,
            memcmp(dice::test::kExpectedCborMldsa65Cert_HashOnlyInput,
                   next_state.certificate,
                   next_state.certificate_size - DICE_SIGNATURE_BUFFER_SIZE));
}
...
TEST(DiceOpsTest, KnownAnswerDescriptorInput) {
  ...
  DiceResult result = DiceMainFlow(
      ...
      &next_state.certificate_size, next_state.cdi_attest, next_state.cdi_seal);
  EXPECT_EQ(kDiceResultOk, result);
  DumpState(CertificateType_Cbor, KeyType_MLDSA_65, "descriptor_input",
            next_state);
  ...
  ASSERT_EQ(sizeof(dice::test::kExpectedCborMldsa65Cert_DescriptorInput),
            next_state.certificate_size);
  EXPECT_EQ(0,
            memcmp(dice::test::kExpectedCborMldsa65Cert_DescriptorInput,
                   next_state.certificate,
                   next_state.certificate_size - DICE_SIGNATURE_BUFFER_SIZE));
}
...
TEST(DiceOpsTest, FullCertChain) {
  ...
  dice::test::CreateFakeUdsCertificate(
      NULL, states[0].cdi_attest, CertificateType_Cbor, KeyType_MLDSA_65,
