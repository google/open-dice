...
TEST(DiceOpsTest, KnownAnswerZeroInput) {
  ...
  DumpState(CertificateType_X509, KeyType_MLDSA_87, "zero_input", next_state);
  ...
  ASSERT_EQ(sizeof(dice::test::kExpectedX509Mldsa87Cert_ZeroInput),
            next_state.certificate_size);

  size_t expected_length = ComputeX509PayloadSize(
      dice::test::kExpectedX509Mldsa87Cert_ZeroInput,
      sizeof(dice::test::kExpectedX509Mldsa87Cert_ZeroInput));
...
TEST(DiceOpsTest, KnownAnswerHashOnlyInput) {
  ...
  DiceResult result = DiceMainFlow(
      ...
      &next_state.certificate_size, next_state.cdi_attest, next_state.cdi_seal);
  EXPECT_EQ(kDiceResultOk, result);
  DumpState(CertificateType_X509, KeyType_MLDSA_87, "hash_only_input",
            next_state);
  // Both CDI values and the certificate should be deterministic.
  EXPECT_EQ(
  ...
  EXPECT_EQ(
      0, memcmp(next_state.cdi_seal, dice::test::kExpectedCdiSeal_HashOnlyInput,
                DICE_CDI_SIZE));
  ASSERT_EQ(sizeof(dice::test::kExpectedX509Mldsa87Cert_HashOnlyInput),
            next_state.certificate_size);

  size_t expected_length = ComputeX509PayloadSize(
      dice::test::kExpectedX509Mldsa87Cert_HashOnlyInput,
      sizeof(dice::test::kExpectedX509Mldsa87Cert_HashOnlyInput));
  size_t actual_length = ComputeX509PayloadSize(next_state.certificate,
                                                next_state.certificate_size);
...
TEST(DiceOpsTest, KnownAnswerDescriptorInput) {
  ...
  DiceResult result = DiceMainFlow(
      ...
      &next_state.certificate_size, next_state.cdi_attest, next_state.cdi_seal);
  EXPECT_EQ(kDiceResultOk, result);
  DumpState(CertificateType_X509, KeyType_MLDSA_87, "descriptor_input",
            next_state);
  // Both CDI values and the certificate should be deterministic.
  EXPECT_EQ(
      0, memcmp(next_state.cdi_attest,
  ...
  ASSERT_EQ(sizeof(dice::test::kExpectedX509Mldsa87Cert_DescriptorInput),
            next_state.certificate_size);

  size_t expected_length = ComputeX509PayloadSize(
      dice::test::kExpectedX509Mldsa87Cert_DescriptorInput,
      sizeof(dice::test::kExpectedX509Mldsa87Cert_DescriptorInput));
  size_t actual_length = ComputeX509PayloadSize(next_state.certificate,
                                                next_state.certificate_size);
...
TEST(DiceOpsTest, NonZeroMode) {
  constexpr size_t kModeOffsetInCert = 0xC43;
  DiceStateForTest current_state = {};
  DiceStateForTest next_state = {};
  DiceInputValues input_values = {};
...
TEST(DiceOpsTest, PartialCertChain) {
  ...
  for (size_t i = 0; i < kNumLayers; ++i) {
    ...
    DumpState(CertificateType_X509, KeyType_MLDSA_87, suffix, states[i + 1]);
  }
...
TEST(DiceOpsTest, FullCertChain) {
  ...
  dice::test::CreateFakeUdsCertificate(
      NULL, states[0].cdi_attest, CertificateType_X509, KeyType_MLDSA_87,
