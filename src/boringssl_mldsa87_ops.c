...
// An implementation of the ML-DSA-87 signature operations using boringssl.
...
#error "ML-DSA-87 needs 2592 bytes to store the public key."
#endif
#if DICE_PRIVATE_KEY_BUFFER_SIZE != 32
#error \
    "This ML-DSA-87 implementation uses 32 bytes from seed for the private key."
#endif
#if DICE_SIGNATURE_BUFFER_SIZE != 4627
#error "ML-DSA-87 needs 4627 bytes to store the signature."
#endif
...
DiceResult DiceKeypairFromSeed(
  ...
  if (1 !=
      MLDSA87_private_key_from_seed(&priv, seed, DICE_PRIVATE_KEY_SEED_SIZE)) {
...
DiceResult DiceVerify(void* context_not_used, const uint8_t* message,
  ...
  if (1 != MLDSA87_verify(&parsed_pub, signature, DICE_SIGNATURE_BUFFER_SIZE,
